export default class ChartPatterns {
    ...
    isBullishFlag() {
        ...
        const flagpoleStart = this.close[length - flagpoleLength * 2];
        const flagpoleEnd = this.close[length - flagpoleLength];
        ...
        const consolidationStart = this.close[length - flagpoleLength];
        const consolidationEnd = this.close[length - 1];
        ...
        for (let i = length - flagpoleLength; i < length - 1; i++) {
            if (this.high[i] > this.high[i + 1] || this.low[i] > this.low[i + 1]) {
                return false;
            }
        }
    ...
    isBullishPennant() {
        ...
        const pennantStart = this.close[length - flagpoleLength];
        const pennantEnd = this.close[length - 1];
    ...
    isDoubleTop() {
        ...
        const firstPeak = Math.max(...this.close.slice(0, midpoint));
        const secondPeak = Math.max(...this.close.slice(midpoint));

        if (firstPeak <= 0 || secondPeak <= 0) {
            return false;
        }

        const firstValley = Math.min(...this.close.slice(0, midpoint));
        const secondValley = Math.min(...this.close.slice(midpoint));
    ...
    isTripleTop() {
        ...
        const firstPeak = Math.max(...this.close.slice(0, third));
        const secondPeak = Math.max(...this.close.slice(third, third * 2));
        const thirdPeak = Math.max(...this.close.slice(third * 2));

        if (firstPeak <= 0 || secondPeak <= 0 || thirdPeak <= 0) {
            return false;
        }

        const dropFromFirstPeak = (firstPeak - Math.min(...this.close.slice(0, third))) / firstPeak;
        const dropFromSecondPeak = (secondPeak - Math.min(...this.close.slice(third, third * 2))) / secondPeak;
        const dropFromThirdPeak = (thirdPeak - Math.min(...this.close.slice(third * 2))) / thirdPeak;
    ...
    isTripleBottom() {
        ...
        const firstValley = Math.min(...this.close.slice(0, third));
        const secondValley = Math.min(...this.close.slice(third, third * 2));
        const thirdValley = Math.min(...this.close.slice(third * 2));

        if (firstValley <= 0 || secondValley <= 0 || thirdValley <= 0) {
            return false;
        }

        const riseFromFirstValley = (Math.max(...this.close.slice(0, third)) - firstValley) / firstValley;
        const riseFromSecondValley = (Math.max(...this.close.slice(third, third * 2)) - secondValley) / secondValley;
        const riseFromThirdValley = (Math.max(...this.close.slice(third * 2)) - thirdValley) / thirdValley;
    ...
    isHeadAndShoulders() {
        ...
        const leftShoulder = Math.max(...this.close.slice(0, third));
        const head = Math.max(...this.close.slice(third, third * 2));
        const rightShoulder = Math.max(...this.close.slice(third * 2));

        if (leftShoulder <= 0 || head <= 0 || rightShoulder <= 0) {
            return false;
        }

        const leftValley = Math.min(...this.close.slice(0, third));
        const middleValley = Math.min(...this.close.slice(third, third * 2));
        const rightValley = Math.min(...this.close.slice(third * 2));
    ...
    isInverseHeadAndShoulders() {
        ...
        const leftShoulder = Math.min(...this.close.slice(0, third));
        const head = Math.min(...this.close.slice(third, third * 2));
        const rightShoulder = Math.min(...this.close.slice(third * 2));

        if (leftShoulder <= 0 || head <= 0 || rightShoulder <= 0) {
            return false;
        }

        const leftPeak = Math.max(...this.close.slice(0, third));
        const middlePeak = Math.max(...this.close.slice(third, third * 2));
        const rightPeak = Math.max(...this.close.slice(third * 2));
    ...
    isBullishTriangle() {
        ...
        const firstHalfHigh = Math.max(...this.high.slice(0, half));
        const secondHalfHigh = Math.max(...this.high.slice(half));

        const firstHalfLow = Math.min(...this.low.slice(0, half));
        const secondHalfLow = Math.min(...this.low.slice(half));
    ...
    isBullishRectangle() {
        ...
        const firstSectionHigh = Math.max(...this.high.slice(0, third));
        const secondSectionHigh = Math.max(...this.high.slice(third, third * 2));
        const thirdSectionHigh = Math.max(...this.high.slice(third * 2));

        const firstSectionLow = Math.min(...this.low.slice(0, third));
        const secondSectionLow = Math.min(...this.low.slice(third, third * 2));
        const thirdSectionLow = Math.min(...this.low.slice(third * 2));
